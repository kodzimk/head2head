...
export const ViewProfile = ({user}: {user: User}) => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await axios.get(`http://20.163.59.127:8000/db/get-user-by-username?username=${username}`)
        const userData = {
          ...initialUser,
          ...
          avatar: response.data.avatar ? `http://20.163.59.127:8000${response.data.avatar}` : undefined
        }
