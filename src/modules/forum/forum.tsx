...
export default function Forum() {
  ...
  // Load real news data from NewsAPI
  useEffect(() => {
    const loadNewsData = async () => {
      try {
        ...
        // Fetch sports headlines for News section
        try {
          const newsResponse = await newsService.getTopSportsHeadlines({
            pageSize: 10
          });
          
          // Store news data in localStorage for news detail page
          localStorage.setItem('forumNewsData', JSON.stringify(newsResponse));
          
          const newsPosts = newsResponse.articles.map(article => 
            newsService.convertToForumPost(article, 'news')
          );
        ...
        try {
          ...
          const transferPosts = transferResponse.articles.map(article => {
  ...
  useEffect(() => {
    ...
    // Sort posts
      ...
      case 'oldest':
        ...
        break;
      case 'popular':
        filtered.sort((a, b) => b.likes - a.likes); // Sort by likes instead of views
        break;
      case 'mostLiked':
  ...
  const handlePostClick = (post: ForumPost) => {
    if (post.type === 'debate') {
      ...
      // Store the specific news data for this post
      const newsData = {
        id: post.id,
        title: post.title,
  ...
  const handleRefreshNews = async () => {
    ...
    try {
      ...
      setNewsError('Failed to refresh news. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Card className="hover:shadow-lg transition-all duration-200 cursor-pointer group">
      ...
      <CardContent className="space-y-3 p-4 sm:p-6" onClick={() => handlePostClick(post)}>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border/50">
          ...
          >
            <Heart className={`w-3 h-3 sm:w-4 sm:h-4 mr-1 ${post.isLiked ? 'fill-current' : ''}`} />
