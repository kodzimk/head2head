...
def generate_ai_quiz(self, battle_id: str, sport: str, level: str, question_count: int = QUESTION_COUNT):
    ...
    try:
        ...
        questions = generate_questions_with_ai(sport, level, question_count, battle_id)
...
def generate_questions_with_ai(sport: str, level: str, question_count: int = QUESTION_COUNT, battle_id: str = None):
    """
    ...
    logger.info(f"Starting AI quiz generation for {sport}/{level}, count={question_count}, battle_id={battle_id}")
    ...
    for attempt in range(max_retries):
        try:
            # Create a model instance with Gemini 2.5 Pro for better performance
            model = genai.GenerativeModel('gemini-2.5-pro')
            
            # Enhanced prompt for better quiz generation with battle-specific uniqueness
            battle_context = f"Battle ID: {battle_id}" if battle_id else "New Battle"
...
"""
            # Generate response
            logger.info(f"Generating AI response for {sport} {level} (attempt {attempt + 1}) with battle_id={battle_id}")
            ...
            try:
                ...
                if not validated_questions:
                    ...
                    validated_questions = generate_expanded_fallback_questions(sport, level, question_count, battle_id)
            ...
            if "429" in error_str or "quota" in error_str.lower() or "rate" in error_str.lower():
                if attempt < max_retries - 1:
                    ...
                    from aiquiz.router import generate_expanded_fallback_questions
                    fallback_questions = generate_expanded_fallback_questions(sport, level, question_count, battle_id)
